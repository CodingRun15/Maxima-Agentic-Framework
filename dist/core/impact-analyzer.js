...
export class ImpactAnalyzer {
     ...
     */
    async analyzeChange(changeRequest) {
        ...
        const mentionedFiles = lockedPaths.filter((fp) => changeRequest.toLowerCase().includes(fp.toLowerCase()));
     ...
     */
    async detectBreakingChanges(changeRequest, lockedFiles) {
        ...
        const hasDangerWord = dangerWords.some((w) => changeRequest.toLowerCase().includes(w));
        const mentionsLockedFile = Object.keys(lockedFiles).some((fp) => changeRequest.toLowerCase().includes(fp.toLowerCase()));
     ...
     */
    async estimateFileChanges(filePath, changeDescription) {
        return {
            file: filePath,
