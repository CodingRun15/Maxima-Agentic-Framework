...
export class AgentClient {
     ...
     */
    async chatStream(config, messages, onStream) {
        try {
            const resolvedConfig = this.resolveProvider(config);
            switch (resolvedConfig.provider) {
                case "anthropic":
    ...
    // ─── Streaming: Anthropic ──────────────────────────────────────────
    async streamAnthropic(config, messages, onStream) {
        if (!this.anthropic)
    ...
    // ─── Streaming: OpenAI ─────────────────────────────────────────────
    async streamOpenAI(config, messages, onStream) {
        if (!this.openai)
    ...
    // ─── Streaming: Google ─────────────────────────────────────────────
    async streamGoogle(config, messages, onStream) {
        if (!this.google)
    ...
    // ─── Streaming: Ollama ─────────────────────────────────────────────
    async streamOllama(config, messages, onStream) {
        if (!this.ollama)
    ...
    resolveProvider(config) {
        ...
        for (const fb of fallbacks) {
            if (providerAvailable[fb.provider]) {
                ...
                logger.warn(`Provider ${config.provider} not available, falling back to ${fb.provider}/${model}`);
    ...
    // ─── Anthropic ─────────────────────────────────────────────────────
    async chatAnthropic(config, messages) {
        if (!this.anthropic) {
        ...
        if (config.thinking) {
            ...
            const usage = {
                ...
                estimatedCostUsd: this.estimateCost(response.usage.input_tokens, response.usage.output_tokens, config),
            };
    ...
    // ─── OpenAI ─────────────────────────────────────────────────────────
    async chatOpenAI(config, messages) {
        if (!this.openai) {
        ...
        const usage = {
            ...
            estimatedCostUsd: this.estimateCost(response.usage?.prompt_tokens ?? 0, response.usage?.completion_tokens ?? 0, config),
        };
        return { content, usage };
    }
    // ─── Google ─────────────────────────────────────────────────────────
    async chatGoogle(config, messages) {
        if (!this.google) {
        ...
        const usage = {
            ...
            estimatedCostUsd: this.estimateCost(response.usageMetadata?.promptTokenCount ?? 0, response.usageMetadata?.candidatesTokenCount ?? 0, config),
        };
        return { content, usage };
    }
    // ─── Ollama ─────────────────────────────────────────────────────────
    async chatOllama(config, messages) {
        try {
            const response = await this.ollama?.chat({
                model: config.model,
                messages: messages.map((m) => ({
            ...
            logger.warn(`Ollama not available (${config.model}), falling back to cloud model.`);
    ...
    // ─── Cost Estimation ──────────────────────────────────────────────
    estimateCost(inputTokens, outputTokens, config) {
        const costTable = {
            // Anthropic — 2026
