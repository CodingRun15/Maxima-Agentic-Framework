...
import { AgentClient } from "../agents/agent-client.js";
...
export class ChatInterface {
    ...
    constructor(projectRoot) {
        ...
        this.contextPacker = new ContextPacker(this.stateManager, this.dependencyAnalyzer);
    ...
    async handleBreakdown(description) {
        if (!description) {
        ...
        // Check budget
        const budgetCheck = await this.budgetManager.checkBudget("breakdown", estimateTokens(prompt));
        ...
        if (cached) {
            ...
            await this.cacheManager.store("breakdown", prompt, [context], result.content, routing.config.model, result.usage);
    ...
    async handleApprovePlan() {
        if (!this.pendingPlan) {
            console.log(chalk.red("No pending plan to approve. Run /breakdown first."));
            return;
        }
    ...
    async handleBuild() {
        ...
        if (!plan || !plan.approved) {
            console.log(chalk.red("No approved plan found. Run /breakdown and /approve-plan first."));
            return;
        }
        console.log(chalk.bold.green("\n🚀 Starting build...\n"));
        for (const phase of plan.phases) {
            ...
            if (phase.parallel && phase.tasks.length > 1) {
                ...
                await Promise.all(phase.tasks.map((task) => this.executeTask(task, plan)));
            }
            else {
                for (const task of phase.tasks) {
            ...
            const approved = await this.askForApproval(`Approve Phase ${phase.id}?`);
            if (!approved) {
    ...
    async executeTask(task, plan) {
        ...
        const lockedDecisions = Object.values(lockedFiles)
            ...
            .join("\n");
        // 3. Build prompt
        const prompt = this.promptBuilder.buildTaskPrompt(task, contextStr, lockedDecisions);
        ...
        // 5. Check budget
        const budgetCheck = await this.budgetManager.checkBudget(task.id, estimateTokens(prompt));
        if (!budgetCheck.allowed) {
            console.log(chalk.red(`  ✗ ${task.name}: ${budgetCheck.reason}`));
            return;
        }
        // 6. Check cache
        const inputContents = await Promise.all(task.files
            .filter(async (f) => await this.stateManager.fileExists(f))
            .map((f) => this.stateManager.readFile(f).catch(() => "")));
        const cached = await this.cacheManager.lookup(task.id, prompt, inputContents);
        let result;
        if (cached) {
            this.budgetManager.recordCacheHit();
            result = { content: cached.output, usage: cached.tokensUsed };
            ...
            // 8. Cache the result
            await this.cacheManager.store(task.id, prompt, inputContents, result.content, routing.config.model, result.usage);
    ...
    async handleChange(description) {
        if (!description) {
        ...
        const lockedSummary = Object.entries(lockedFiles)
            .map(([fp, info]) => `${fp} (${info.lines} lines, exports: ${info.exports?.join(", ") || "none"})`)
            .join("\n");
        const prompt = this.promptBuilder.buildImpactAnalysisPrompt(description, lockedSummary, packed.architectureSummary);
        ...
        const cached = await this.cacheManager.lookup("impact-analysis", prompt, [lockedSummary]);
        let result;
        if (cached) {
            this.budgetManager.recordCacheHit();
            result = { content: cached.output, usage: cached.tokensUsed };
            ...
            await this.cacheManager.store("impact-analysis", prompt, [lockedSummary], result.content, routing.config.model, result.usage);
            await this.budgetManager.recordUsage("impact-analysis", routing.tier, result.usage);
        }
        console.log(chalk.bold("\n📋 IMPACT ANALYSIS:\n"));
        console.log(result.content);
    }
    // ─── /brainstorm — Interactive brainstorming mode ─────────────────
    async handleBrainstorm(topic) {
        if (!topic) {
        ...
        const result = await this.agentClient.chatStream(routing.config, this.brainstormHistory, (chunk) => process.stdout.write(chunk));
    ...
    async handleBrainstormMessage(message) {
    ...
    async handleMessage(message) {
        ...
        const result = await this.agentClient.chatStream(routing.config, this.conversationHistory, (chunk) => process.stdout.write(chunk));
        process.stdout.write("\n\n");
    ...
    handleTierOverride(argument) {
        const valid = ["thinking", "expensive", "cheap", "local"];
        ...
        console.log(chalk.green(`✓ All requests will now use ${tierEmoji[tier] || ""} ${tier} tier`));
