...
export class ContextPacker {
    stateManager;
    dependencyAnalyzer;
    constructor(stateManager, dependencyAnalyzer) {
     ...
     */
    async packForTask(task, allTasks) {
        const files = [];
        // 1. Get dependency-aware file list (only task files + immediate imports)
        const relevantPaths = this.dependencyAnalyzer.getTaskDependencyFiles(task, allTasks);
        ...
        const totalTokenEstimate = files.reduce((sum, f) => sum + f.tokenEstimate, 0) +
            estimateTokens(architectureSummary);
        logger.debug(`Context packed: ${files.length} files, ~${totalTokenEstimate} tokens ` +
