...
export class TokenBudgetManager {
     ...
     */
    async checkBudget(taskId, estimatedInputTokens) {
     ...
     */
    async recordUsage(taskId, tier, usage) {
     ...
     */
    estimateCost(promptText, tier, expectedOutputTokens = 2000) {
        ...
        const estimatedCostUsd = (inputTokens + expectedOutputTokens) * costPerToken[tier];
    ...
    async printCostSummary() {
        ...
        for (const [tier, usage] of Object.entries(report.byTier)) {
            if (usage.inputTokens > 0 || usage.outputTokens > 0) {
                console.log(`    ${tier.padEnd(10)} in:${usage.inputTokens} out:${usage.outputTokens} $${usage.estimatedCostUsd.toFixed(4)}`);
            }
        }
        console.log("");
        console.log("  Lifetime:");
        console.log(`    Total tokens:  ${budget.lifetimeUsage.inputTokens + budget.lifetimeUsage.outputTokens}`);
        console.log(`    Total cost:    $${budget.lifetimeUsage.estimatedCostUsd.toFixed(4)}`);
    }
}
