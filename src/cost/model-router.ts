import {
    AgentConfig, AgentType, ModelConfig, ModelTier, RoutingDecision, TaskComplexity,
} from "../core/types.js";
...
const KEYWORD_COMPLEXITY: Array<{ keywords: string[]; complexity: TaskComplexity }> = [
    {
...
export class ModelRouter {
     ...
     */
    async routeChat(): Promise<RoutingDecision> {
        const config = await this.stateManager.getModelConfig();
     ...
     */
    async routeExpensive(name: string): Promise<RoutingDecision> {
        const config = await this.stateManager.getModelConfig();
     ...
     */
    async routeThinking(name: string): Promise<RoutingDecision> {
        const config = await this.stateManager.getModelConfig();
     ...
     */
    async routeForced(name: string, tier: ModelTier): Promise<RoutingDecision> {
        const config = await this.stateManager.getModelConfig();
        const modelPair = config[tier];
